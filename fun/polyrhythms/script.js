...
const draw = () => {
  ...
  const arcs = [
    ...
    // Animation code for dots
    const oneFullLoop = 2 * Math.PI,
          numberOfLoops = maxLoops - index,
          maxAngle = 2 * Math.PI,
          velocity = (oneFullLoop * numberOfLoops) / duration,
    ...
    // Probably could've broken this up more
    return {
      color,
      audio,
  ...
  // Does the animation for each arc
  arcs.forEach((arc, index) => {
    ...
    // Places dots on arc
    const x = center.x + arcRadius * Math.cos(arc.adjustedDistance),
          y = center.y + arcRadius * Math.sin(arc.adjustedDistance);
...
document.onvisibiliychange = () => soundEnabled = false;
...
paper.onclick = () => sounEnabled = !soundEnabled;
...
const draw = () => {
  ...
  const arcs = [
    ...
    const audio = new Audio(`https://${index}');
  ...
  arcs.forEach((arc, index) => {
    const arc Radius = initialArcRadius + (index * spacing);
  });

  // Draw arc
  pen.beginPath();
  pen.strokeStyle = arc;
  pen.arc(center.x, center.y, lenth * 0.05, Math.PI, 2* Math.PI);
  pen.stroke();
  ...
  const oneFullLoop = 2* Math.PI,
        ...
        distance = Math.PI + (elapsedTime * velocity),
        modDistance = distance % maxAngle,
        adjustedDisance = modDistance >= Math.PI ? distance : maxAngle * modDistance;
  ...
  pen.fillStyle = "white";
  pen.beginPath();
  pen.arc(x, y, lenth * 0.0065, 0, 2 * Math.PI);
  pen.fill();
  pen.stroke();
